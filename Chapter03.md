...
```ruby
 n = group.order                                      # Gの位数
 prvKey = 1 + SecureRandom.random_number(n-1)         # 秘密鍵の生成
 pubKey = group.generator.multiply_by_scalar(prvKey)  # 公開鍵の生成
 pubKey.x                                             # 公開鍵のx座標の値
 pubKey.y                                             # 公開鍵のy座標の値
```
...
```ruby
 ...
 prvKey = key.private_key.to_i                 # 秘密鍵を整数で表示
...
```ruby
 ...
 sign = ECDSA.sign(group,prvKey,digest,k)            # 電子署名の作成
